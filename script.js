const questions = [
    {
        ...
        answers: [
            ["具體時間、地點和順序都要排好，照行程表執行（09:00 到 A 景點、11:30 到 B 景點）", "嚴格照表型"],
            ["先排重要項目，地點和順序可以現場調整（只先決定今天一定要去的幾個地方）", "重點規劃型"],
            ["只要有大方向，細節到現場再決定（到了當地再看要去哪裡）", "彈性調整型"],
            ["不太想安排，跟著旅伴走就好（由對方決定每天去哪裡）", "跟隨型"],
        ],
    },
    {
        tag: "SCENIC ROUTE",
        title: "如果安排一段需要配合班次、沿線散策的旅程，你會怎麼看？",
        hint: "搭乘地方交通工具，沿途下車走走。",
        answers: [
            ["每站要待多久、要遊覽哪些點都先安排清楚（事先決定每站停留時間和必看景點）", "逐站規劃型"],
            ["大致安排每站的停留點，具體行程可以調整（到了現場再決定先逛哪一個地方）", "彈性規劃型"],
            ["只要沿途風景好，細節不用先想（搭上車再決定要去哪一站）", "完全隨性型"],
            ["希望旅伴全部安排好，我跟著走（只要告訴我集合時間和地點）", "跟隨型"],
        ],
    },
    {
        tag: "TIME & PATIENCE",
        ...
        answers: [
            ["直接提醒他，之後要準時（說明再遲到就會錯過下一班車，後面景點也會被迫取消）", "明確型"],
            ["先等一下，但心裡會不爽（嘴上說沒關係，心裡開始計算後面要刪掉哪些行程）", "在意型"],
            ["沒關係，自己先逛附近（先找咖啡店等，後面的景點再視情況調整）", "彈性型"],
            ["覺得旅行不用看時間（錯過預約就改去別的地方，不把它當成問題）", "隨意型"],
        ],
    },
    {
        tag: "MONEY MATTERS",
        ...
        answers: [
            ["每筆支出都記錄，最後精準平分（門票、餐費和飲料都各自記帳）", "精準分帳型"],
            ["大筆支出先共同確認，小額用輪流或大概平衡（住宿先講好，咖啡不用立刻拆帳）", "事前共識型"],
            ["為了舒適和體驗可以提高預算（願意多花錢住好一點或吃想吃的餐廳）", "品質優先型"],
            ["先設定總預算，超過就不考慮（改住遠一點、少搭計程車或減少付費行程）", "預算控制型"],
        ],
    },
    {
        tag: "FOOD OPINION",
        ...
        answers: [
            ["一定一起吃飯，大家一起投票，少數服從多數（比較餐廳後共同決定一間）", "共同決策型"],
            ["這餐配合一個人，下餐再換另一個人選（今天吃麵，明天吃燒肉）", "輪流配合型"],
            ["各自去吃想吃的，約好時間和地點再會合（吃完一起去下一個景點）", "分開用餐型"],
            ["直接選一個自己最想吃的，希望大家一起配合（不想為了投票放棄原本最想吃的店）", "主導選擇型"],
        ],
    },
    {
        tag: "PERSONAL SPACE",
        ...
        answers: [
            ["白天和晚上都一起行動、一起休息（從早餐到睡前都不分開）", "全程同行型"],
            ["白天一起走，晚上各自保留一小段時間（各自滑手機或放空半小時）", "短暫充電型"],
            ["白天也需要半天或一天各自行動（安排一整段自由活動）", "高度獨立型"],
            ["白天可以一起行動，但住宿要有明確的個人空間（分房或各自有安靜的房間）", "住宿分開型"],
        ],
    },
    {
        tag: "INTEREST & EFFORT",
    ...
    },
    {
        ...
        hint: "逛老街、看地方店家、喝咖啡，不追景點清單。",
        answers: [
            ["可以，而且會把散策本身當成這趟旅行的主要行程（慢慢走一整天也沒問題）", "深度散策型"],
            ["可以，但希望安排一兩個明確亮點（散策中穿插一間特色店）", "平衡型"],
            ["只能接受短時間散策，不希望整天都沒有明確目的（半天後要安排其他活動）", "有限接受型"],
            ["不太能接受，旅行應該要有值得去的景點（希望每天至少有一個主景點）", "目的型"],
        ],
    },
    {
        tag: "INTEREST GAP",
        title: "如果旅伴對某個景點沒興趣，你會怎麼處理？",
        hint: "你很想去，但對方覺得「看起來還好」。",
        answers: [
            ["一起討論，找出兩個人都能接受的方式（縮短停留，再安排對方喜歡的地方）", "協調型"],
            ["你去你的，他去附近想去的地方（約好一個時間和地點再會合）", "分開型"],
            ["先陪對方去，再換一個他有興趣的景點（這站配合你、下一站配合他）", "輪流型"],
            ["既然他沒興趣，就直接刪掉這個景點（改去兩個人都想去的地方）", "配合型"],
        ],
    },
    {
        tag: "ACCOMMODATION",
        ...
        answers: [
            ["乾淨、安全就可以，其他不用太講究（房間小一點也沒關係）", "實用型"],
            ["地點和交通方便最重要（下車後能快速到住宿）", "便利型"],
            ["價格優先，能省就省（願意住遠一點或接受基本設備）", "省錢型"],
            ["舒適、安靜，而且需要明確的個人空間（希望有好的床、隔音或獨立房間）", "品質型"],
        ],
    },
    {
        tag: "SHARED SPACE",
    ...
    },
    {
        ...
        answers: [
            ["行程已約好，卻常常臨時遲到或臨時改時間（讓大家錯過預約或班次）", "時間失信"],
            ["事前完全不參與討論，到了現場卻否定已安排好的行程（一直說「早知道不要來」）", "事後批評"],
            ["每一筆小額花費都要反覆計算，讓整趟旅程一直在對帳（連一瓶水也要立刻拆帳）", "過度計較"],
            ["自己不查交通、不做決定，出問題時卻把責任推給別人（迷路後只怪別人沒帶路）", "逃避責任"],
        ],
    },
];
let current = 0,
